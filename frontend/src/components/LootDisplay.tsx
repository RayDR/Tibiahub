...
const LootDisplay: React.FC<LootDisplayProps> = ({ items }) => {
  ...
  if (sortedItems.length === 0) {
    return <div className="text-sm text-slate-500">No drop data available.</div>;
  }
    ...
    <div className="space-y-4">
          ...
          <div key={loot.id} className="rounded-xl border border-slate-800 bg-slate-950/50 p-4">
            <div className="mb-3 flex items-center gap-3">
              <ImageWithFallback
                ...
                alt={loot.item_name}
                className="h-12 w-12 rounded-lg object-contain bg-slate-800"
                containerClassName="h-12 w-12"
                fallbackLabel="Item"
              />
              <div className="min-w-0">
                <div className="truncate font-semibold text-slate-100">{loot.item_name}</div>
      ...
      {!showAll && sortedItems.length > 20 && (
        <button
          ...
          className="w-full rounded-lg border border-amber-500/30 px-4 py-2 text-sm text-amber-300 transition hover:bg-amber-500/10"
        >
