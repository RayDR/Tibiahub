...
function currentContext(pathname: string, t: (key: string) => string): { parent?: { label: string; to: string }; label: string } | null {
  ...
  if (pathname === '/npcs') return { parent: { label: t('nav.search'), to: '/cyclopedia?tab=npcs' }, label: t('nav.npcs') };
  if (pathname.startsWith('/npcs/')) return { parent: { label: t('nav.npcs'), to: '/cyclopedia?tab=npcs' }, label: t('shell.context.npc') };
