import { BookOpen, ChevronDown, Loader2, MapPin, X } from 'lucide-react';
...
import { hasDetailedQuestData, hasDetailedQuestSummary } from '../../utils/questPresentation';
...
export default function QuestPreviewDialog({
  ...
  const topLevelRelationships = detail?.relationships.filter((relationship) => !relationship.mission_id) || [];
  const hasRequirements = Boolean(detail && (detail.required_items.length || detail.required_quests.length));
  const hasRewards = Boolean(detail?.rewarded_items.length);
  const hasMissions = Boolean(detail?.missions.length);
  ...
  return <Dialog open={open} onClose={onClose} label={t('questDetail.previewTitle', { name: quest.name })} className="quest-preview-dialog quest-codex w-[min(94vw,46rem)] overflow-hidden p-0">
    ...
    <div className="quest-codex__pages p-5 sm:p-7">
      ...
      <div className="mt-5 flex flex-wrap gap-2 text-xs">
        ...
        {detail?.duration ? <span className="quest-codex__chip rounded-full border px-2.5 py-1">{t('questEnhancement.duration')}: {detail.duration}</span> : null}
      ...
      {!loading && !error && detail && hasDetails ? (
        <div className="mt-5 grid gap-3">
          {hasMissions ? (
            <details className="quest-codex__preview-section rounded-xl border p-3">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-sm font-semibold">
                <span>{t('questEnhancement.missionIndex')} ({detail.missions.length})</span>
              ...
              <ol className="mt-3 space-y-1.5">
                {detail.missions.map((mission) => (
                  <li key={mission.id}>
                    <Link
                      to={`${route}#mission-${mission.id}`}
                      state={linkState}
                      onClick={() => { onClose(); onNavigate?.(); }}
                      className="quest-codex__toc-link flex min-h-10 items-center gap-2 rounded-lg border px-3 py-2 text-sm"
                    >
                      <span className="font-serif font-bold">{mission.sequence}</span>
          ...
          {hasRequirements ? (
            <section className="quest-codex__preview-section rounded-xl border p-3">
              <h3 className="text-sm font-semibold">{t('questEnhancement.requirementItems')}</h3>
              {detail.required_items.length ? (
                <div className="mt-2 grid gap-2 sm:grid-cols-2">
                  {detail.required_items.map((item, index) => <QuestReference key={`${item.name}-${index}`} value={item} kind="item" relationships={topLevelRelationships} linkState={linkState} previewable={false} compact />)}
                </div>
              ) : null}
              {detail.required_quests.length ? (
                <div className="mt-3">
                  <p className="mb-2 text-xs font-semibold uppercase tracking-wide">{t('questEnhancement.prerequisiteQuests')}</p>
                  <div className="grid gap-2 sm:grid-cols-2">
                    {detail.required_quests.map((requiredQuest, index) => <QuestReference key={`${requiredQuest.name}-${index}`} value={requiredQuest} kind="quest" relationships={topLevelRelationships} linkState={linkState} previewable={false} compact />)}
                  </div>
                </div>
              ) : null}
            </section>
          ...
          {hasRewards ? (
            <section className="quest-codex__preview-section rounded-xl border p-3">
              <h3 className="text-sm font-semibold">{t('questEnhancement.rewardItems')}</h3>
              <div className="mt-2 grid gap-2 sm:grid-cols-2">
                {detail.rewarded_items.map((item, index) => <QuestReference key={`${item.name}-${index}`} value={item} kind="item" relationships={topLevelRelationships} linkState={linkState} previewable={false} compact />)}
              </div>
            </section>
          ) : null}
        </div>
