...
export default function HuntZoneCard({ zone, linkState, onNavigate, rawExperience, score, onInspectMap }: HuntZoneCardProps) {
  ...
  return <article data-hunt-zone-card className="group relative isolate flex min-h-[21rem] overflow-hidden rounded-2xl border border-line bg-surface-raised shadow-sm transition hover:-translate-y-0.5 hover:border-primary/50 hover:shadow-xl motion-reduce:transform-none">
    ...
    <div className="absolute inset-0 bg-gradient-to-b from-surface-overlay/20 via-surface-overlay/55 to-surface-overlay/95" />
    <div className="relative flex min-w-0 flex-1 flex-col p-4 text-content-primary sm:p-5">
      <div className="flex min-h-7 items-start justify-between gap-3">
        {mapped ? <span className="inline-flex items-center gap-1 rounded-full border border-line bg-surface-overlay/70 px-2 py-1 text-[11px] font-semibold backdrop-blur"><Map size={12} />{t('map.mapped')}</span> : <span className="inline-flex items-center gap-1 rounded-full border border-line bg-surface-overlay/75 px-2 py-1 text-[11px] text-content-secondary backdrop-blur"><MapPinOff size={12} />{t('map.locationNotMapped')}</span>}
      ...
      <div className="mt-auto pt-16">
        <div className="min-h-[4.25rem]">
          ...
          <p className="mt-1 min-h-5 truncate text-sm text-content-secondary">{[place, zone.spatial?.z != null ? t('map.floor', { floor: formatDisplayFloor(zone.spatial.z) }) : null].filter(Boolean).join(' · ') || t('map.locationNotMapped')}</p>
        </div>
        ...
        <div className="mt-4 grid min-h-9 grid-cols-2 gap-2">
          <Link to={`/planner?zone=${encodeURIComponent(String(identifier))}`} className="inline-flex min-h-9 items-center justify-center gap-1.5 rounded-lg border border-line bg-surface-overlay/65 px-3 text-xs font-semibold text-content-primary backdrop-blur hover:bg-surface-overlay/90"><Route size={14} />{t('cyclopedia.zones.comparePlanner')}</Link>
