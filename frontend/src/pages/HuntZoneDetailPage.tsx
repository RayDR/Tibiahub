...
import { buildMapEntityUrl, tibiaMapApi, type HuntZoneMapContext } from '../services/tibiaMap';
...
export default function HuntZoneDetailPage() {
  ...
  return <Page>
    ...
    <KnowledgeSection className="mt-6" title={t('huntZoneDetail.map')} icon={<MapPinned size={20} />}>
      {hasMap && spatial && worldMap ? <div className="overflow-hidden rounded-2xl"><Suspense fallback={<div className="grid min-h-72 place-items-center text-content-muted">{t('map.loading')}</div>}><TibiaMapViewer imageUrl={worldMap.image_url} pathfindingUrl={worldMap.pathfinding_url} label={zone.name} floor={spatial.z} floorLabel={spatial.z != null ? t('map.floor', { floor: formatDisplayFloor(spatial.z) }) : undefined} mapBounds={worldMap.bounds} center={{ x: spatial.x as number, y: spatial.y as number }} focusBounds={spatial.bounds ? { minX: spatial.bounds.min_x, minY: spatial.bounds.min_y, maxX: spatial.bounds.max_x, maxY: spatial.bounds.max_y } : undefined} markers={(mapContext?.markers || []).map((marker) => ({ x: marker.x, y: marker.y, label: marker.name, imageUrl: marker.image_url }))} regions={spatial.bounds ? [{ minX: spatial.bounds.min_x, minY: spatial.bounds.min_y, maxX: spatial.bounds.max_x, maxY: spatial.bounds.max_y, label: zone.name }] : []} paths={(mapContext?.routes || []).map((route) => ({ id: route.id, label: route.name, points: route.points }))} coordinateMode="world" resetLabel={t('map.reset')} zoomInLabel={t('map.zoomIn')} zoomOutLabel={t('map.zoomOut')} emptyMessage={t('huntZoneDetail.mapEmpty')} /></Suspense><div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-xs text-content-muted"><span>{t('huntZoneDetail.coordinates', { x: spatial.x, y: spatial.y, z: spatial.z != null ? formatDisplayFloor(spatial.z) : t('common.unknown') })}</span><Link to={buildMapEntityUrl({ canonicalEntityId: zone.knowledge_entity_id, entityType: 'hunt_zone', name: zone.name, slug: zone.slug || String(zone.id), floor: spatial.z })} className="app-button-secondary app-button-sm">{t('huntZoneDetail.viewMap')}</Link></div></div> : <KnowledgeEmpty>{t('map.locationNotMapped')}</KnowledgeEmpty>}
    </KnowledgeSection>
