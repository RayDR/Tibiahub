...
export default function QuestDetailPage() {
  ...
  return <Page><div>
    ...
    <article className="relative overflow-hidden rounded-3xl border border-line-strong bg-surface-base/70 shadow-sm">
      <div className="h-2 bg-gradient-to-r from-primary-active via-primary to-primary-active" />
      <div className="p-5 sm:p-8 lg:p-10">
      <header id="overview" className="scroll-mt-24 border-b border-line pb-8 text-center"><ScrollText className="mx-auto text-primary" size={36} /><p className="mt-3 text-xs font-bold uppercase tracking-[0.25em] text-primary">{t('questDetail.codexEntry')}</p><h1 className="mx-auto mt-3 max-w-4xl font-serif text-3xl font-bold text-content-primary sm:text-5xl">{quest.name}</h1>{quest.group_name && <p className="mt-2 text-sm text-primary">{t('questDetail.group', { name: quest.group_name })}</p>}<p className="mx-auto mt-6 max-w-3xl text-left text-lg leading-8 text-content-secondary sm:text-center">{quest.summary || quest.description || t('questDetail.noDetails')}</p><Link to={`/map?entityType=quest&slug=${encodeURIComponent(quest.slug || quest.name)}&q=${encodeURIComponent(quest.name)}`} className="app-button-secondary app-button-sm mt-4 inline-flex"><MapPin size={14} />{t('map.openDetails')}</Link></header>
