...
export default function KnowledgeOperations() {
  ...
  const enqueue = async () => {
    if (!selectedProvider || !enqueueEntity || !enqueueJobType || !canonicalName.trim() || !languageNeutralId.trim()) return;
    setBusyJob('enqueue');
    try {
      const result = await knowledgeOperationsApi.enqueue({
        ...
        scope: {},
        payload: { canonical_name: canonicalName.trim(), language_neutral_id: languageNeutralId.trim(), provider_document_id: languageNeutralId.trim() },
      });
      toast.success(t(result.created ? 'knowledgeOps.messages.enqueued' : 'knowledgeOps.messages.alreadyActive'));
      setCanonicalName(''); setLanguageNeutralId('');
      await loadJobs();
  ...
  return <div className="space-y-5">
    ...
    <section className="space-y-3 rounded-xl border border-slate-700 bg-slate-900/30 p-4"><h3 className="font-medium text-slate-200">{t('knowledgeOps.enqueue.title')}</h3><p className="text-xs text-slate-400">{t('knowledgeOps.enqueue.help')}</p>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <select aria-label={t('knowledgeOps.fields.jobType')} value={enqueueJobType} onChange={event => setEnqueueJobType(event.target.value)} className="min-h-11 rounded-lg border border-slate-700 bg-slate-950 px-3 text-sm"><option value="">{t('knowledgeOps.fields.jobType')}</option>{(selectedProvider?.supported_job_types || []).map(jobType => <option key={jobType} value={jobType}>{jobType}</option>)}</select>
        <input value={canonicalName} onChange={event => setCanonicalName(event.target.value)} placeholder={t('knowledgeOps.fields.canonicalName')} aria-label={t('knowledgeOps.fields.canonicalName')} className="min-h-11 rounded-lg border border-slate-700 bg-slate-950 px-3 text-sm" />
        <input value={languageNeutralId} onChange={event => setLanguageNeutralId(event.target.value)} placeholder={t('knowledgeOps.fields.languageNeutralId')} aria-label={t('knowledgeOps.fields.languageNeutralId')} className="min-h-11 rounded-lg border border-slate-700 bg-slate-950 px-3 text-sm" />
      </div><button onClick={() => void enqueue()} disabled={busyJob === 'enqueue' || !selectedProvider || !enqueueEntity || !enqueueJobType || !canonicalName.trim() || !languageNeutralId.trim()} className="flex min-h-11 items-center gap-2 rounded-lg bg-amber-600 px-4 text-sm font-medium text-white disabled:opacity-50">{busyJob === 'enqueue' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}{t('knowledgeOps.actions.enqueue')}</button>
