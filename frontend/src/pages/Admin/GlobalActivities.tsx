...
export default function GlobalActivities() {
  ...
  return <div className="space-y-4"><WorkspaceHeader title={t('raffle.global.title')} subtitle={t('raffle.global.subtitle')} badge={t('workspace.admin.badge')} /><RaffleCreationWizard isGlobalAdmin onCreated={() => void load()} />{error ? <EmptyState title={t('raffle.workspace.errors.title')} description={t('raffle.workspace.errors.help')} /> : <div className="grid gap-3 sm:grid-cols-2">{items.map(item => <article key={item.id} className="rounded-xl border border-slate-800 p-4"><span className="rounded-full bg-sky-500/15 px-2 py-1 text-xs text-sky-200">{t(`raffle.workspace.scopes.${item.scope_type}.title`)}</span><h2 className="mt-3 font-semibold">{item.title}</h2><p className="text-sm text-slate-400">{item.world_name || t('raffle.global.crossServer')}</p></article>)}</div>}</div>;
}
