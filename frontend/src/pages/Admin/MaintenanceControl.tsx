...
import { AppButton, Badge, Card, FormField, Input, Textarea } from '../../components/ui';
...
export default function MaintenanceControl() {
  ...
  return <div className="workspace-page">
    <WorkspaceContentHeader title={t('maintenanceMode.admin.title')} description={t('maintenanceMode.admin.subtitle')} icon={<Wrench />} action={<AppButton variant="secondary" onClick={() => void load()}><RefreshCw className="size-4" />{t('maintenanceMode.refresh')}</AppButton>} />
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <Card className="p-5"><h2 className="font-semibold">{t('maintenanceMode.holds')}</h2><div className="mt-4 space-y-3">{status?.holds?.length ? status.holds.map(hold => <div key={hold.id} className="rounded-xl border border-line p-3"><div className="flex items-center justify-between gap-2"><Badge tone={hold.hold_type === 'sync' ? 'primary' : 'warning'}>{t(`maintenanceMode.holdTypes.${hold.hold_type}`, hold.hold_type)}</Badge><span className="text-xs text-content-muted">#{hold.id}</span></div><p className="mt-2 text-sm">{hold.reason}</p><p className="mt-1 text-xs text-content-muted">{formatDateTime(hold.enabled_at, i18n.resolvedLanguage || i18n.language)}{hold.owner_job_id ? ` · ${hold.owner_job_id}` : ''}</p><AppButton className="mt-3" size="sm" variant="secondary" onClick={() => void release(hold.id)}>{t('maintenanceMode.release')}</AppButton></div>) : <p className="text-sm text-content-muted">{t('maintenanceMode.noHolds')}</p>}</div></Card>
    </div>
