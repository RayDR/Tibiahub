import { AlertTriangle, CheckCircle2, Loader2, Play, RefreshCw, RotateCcw, Square } from 'lucide-react';
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-[18rem_minmax(0,1fr)]"><Card className="p-4"><div className="flex items-center justify-between"><h2 className="font-semibold">{t('fullSync.operations')}</h2><button aria-label={t('fullSync.refresh')} onClick={() => void load()}><RefreshCw className="size-4" /></button></div><div className="mt-3 space-y-2">{jobs.filter(job => job.job_type === 'full').map(job => <button key={job.job_id} onClick={() => setSelectedId(job.job_id)} className={`w-full rounded-xl border p-3 text-left ${job.job_id === selected?.job_id ? 'border-primary bg-primary-subtle' : 'border-line bg-surface-base'}`}><span className="block truncate text-sm font-medium">{job.operation_label || job.job_id}</span><span className="mt-1 flex items-center justify-between text-xs text-content-muted"><span>{job.operation_status}</span><span>{job.progress_percent}%</span></span></button>)}</div></Card>
      <Card className="p-5">{!selected ? <p className="text-content-muted">{t('fullSync.none')}</p> : <><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">{selected.operation_label || selected.job_id}</h2><p className="mt-1 text-xs text-content-muted">{selected.job_id} · {selected.worker_id || t('fullSync.unclaimed')}</p></div><div className="flex items-center gap-2"><Badge tone={tone(selected.operation_status)}>{selected.operation_status}</Badge>{selected.maintenance_active && <Badge tone="warning">{t('fullSync.maintenanceActive')}</Badge>}</div></div>
        ...
        <div className="mt-5 space-y-3">{selected.phases.map(phase => <div key={phase.id} className="rounded-xl border border-line p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2">{phase.status === 'completed' ? <CheckCircle2 className="size-4 text-success" /> : phase.status === 'running' ? <Loader2 className="size-4 animate-spin text-primary" /> : phase.status === 'failed' ? <AlertTriangle className="size-4 text-danger" /> : null}<span className="font-medium">{t(`fullSync.phases.${phase.phase_key}`)}</span><Badge>{phase.provider || t('fullSync.local')}</Badge></div><Badge tone={tone(phase.status)}>{phase.status}</Badge></div><dl className="mt-3 grid gap-2 text-xs text-content-secondary sm:grid-cols-4"><div><dt>{t('fullSync.processed')}</dt><dd>{phase.processed_count}</dd></div><div><dt>{t('fullSync.failed')}</dt><dd>{phase.failed_count}</dd></div><div><dt>{t('fullSync.offset')}</dt><dd>{phase.current_offset}</dd></div><div><dt>{t('fullSync.attempt')}</dt><dd>{phase.attempt_count}/{phase.max_attempts}</dd></div></dl>{phase.current_entity && <p className="mt-2 text-xs text-content-muted">{t('fullSync.entity')}: {phase.current_entity}</p>}{phase.next_retry_at && <p className="mt-2 text-xs text-warning">{t('fullSync.nextRetry')}: {new Date(phase.next_retry_at).toLocaleString()}</p>}{phase.safe_error && <p className="mt-2 text-xs text-danger">{phase.safe_error} ({phase.error_category})</p>}{Object.keys(phase.checkpoint).length > 0 && <details className="mt-2"><summary className="cursor-pointer text-xs font-medium">{t('fullSync.checkpoint')}</summary><pre className="mt-1 overflow-auto rounded-lg bg-surface-base p-2 text-xs">{JSON.stringify(phase.checkpoint, null, 2)}</pre></details>}{['failed', 'cancelled', 'skipped'].includes(phase.status) && <div className="mt-3 flex gap-2"><AppButton size="sm" variant="secondary" onClick={() => void resumePhase(phase.phase_key)}>{t('fullSync.retryPhase')}</AppButton>{!phase.required && phase.status !== 'skipped' && <AppButton size="sm" variant="ghost" onClick={() => void skipPhase(phase.phase_key)}>{t('fullSync.skip')}</AppButton>}</div>}</div>)}</div>
