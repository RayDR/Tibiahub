...
export default function SyncPhaseCard({ jobId, phase, retryAllowed, onRetry, onSkip }: {
  jobId: string;
  phase: SyncPhase;
  ...
  const blindRetryBlocked = phase.phase_key === 'images' && failurePercent > 80;
  ...
  return <div className={`rounded-xl border p-4 ${highFailure ? 'border-danger bg-danger-subtle' : 'border-line'}`}>
    ...
    {highFailure && <p className="mt-3 rounded-lg border border-danger p-2 text-xs text-danger">{t(blindRetryBlocked ? 'fullSync.errorViewer.canaryRequired' : 'fullSync.errorViewer.highFailure')}</p>}
    ...
    {(phase.failed_count > 0 || phase.last_error) && <AppButton className="mt-3" size="sm" variant="secondary" onClick={() => setErrorsOpen(true)}>{t('fullSync.errorViewer.view')}</AppButton>}
