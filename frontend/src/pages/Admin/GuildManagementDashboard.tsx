...
export default function GuildManagementDashboard() {
  ...
  useEffect(() => {
    if (assistedGuild) void loadMembers(assistedGuild);
  }, [assistedGuild]);
  ...
  const handleUpdateCharacter = async (userId: number) => {
    ...
    try {
      const result = await guildManagementApi.updateUserCharacter(
        userId,
      ...
      if (result.validation_passed) {
        ...
        toast.warning?.(
          `Character updated but validation failed: ${result.validation_message || "Unknown"}`,
        );
      }
  ...
  const handleDeleteEvent = async (eventId: number) => {
    if (!(await confirmation.confirm("Delete this event?", { danger: true })))
      return;
    try {
      const token = localStorage.getItem("token");
  ...
  const handleDeleteAnnouncement = async (announcementId: number) => {
    if (
      !(await confirmation.confirm("Delete this announcement?", {
        danger: true,
      }))
    )
      return;
    ...
    <div className="space-y-4">
      ...
      <div className="admin-panel rounded-xl p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-content-secondary">
                Guild management — members, events, announcements
              </p>
            </div>
          </div>
        ...
        {syncResult && (
          <div className="mt-3 rounded-lg border border-success/40 bg-success/10 px-4 py-2 text-sm text-success flex flex-wrap gap-4">
            <span>
              Synced: <strong>{syncResult.synced_users}</strong>
            </span>
            <span>
              Updated chars: <strong>{syncResult.updated_characters}</strong>
            </span>
            <span>
              Total members: <strong>{syncResult.total_members}</strong>
            </span>
            <span>
              Unlinked accounts: <strong>{syncResult.unlinked_users}</strong>
            </span>
          </div>
        )}
      </div>
      ...
      {loadingContent ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-surface-base/50">
                <tr>
                  <th className="text-left p-3 text-xs font-semibold uppercase tracking-wide text-content-secondary">
                    User
                  </th>
                  <th className="text-left p-3 text-xs font-semibold uppercase tracking-wide text-content-secondary">
                    Rank
                  ...
                  <th className="text-right p-3 text-xs font-semibold uppercase tracking-wide text-content-secondary">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody>
                {members.length === 0 ? (
                  ...
                  members.map((member) => (
                    <tr
                      key={member.id}
                      className="border-t border-line hover:bg-surface-base/30 group"
                    >
                      <td className="p-3">
                        {editingUser === member.id ? (
                          ...
                          <div>
                            ...
                            {member.is_superuser && (
                              <span className="text-xs bg-danger/15 text-danger px-1.5 py-0.5 rounded">
                                Admin
                              </span>
                            )}
                          </div>
                      ...
                      <td className="p-3">
                        {editingUser === member.id ? (
                          <input
                            type="text"
                            value={
                              editForm.guild_rank || member.guild_rank || ""
                            }
                            onChange={(e) =>
                              setEditForm({
                                ...editForm,
                                guild_rank: e.target.value,
                              })
                            }
                            className="bg-surface-base border border-line rounded px-2 py-1 text-sm text-content-primary w-full"
                          />
                      ...
                      <td className="p-3">
                        {editingCharacter === member.id ? (
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() =>
                                void handleUpdateCharacter(member.id)
                              }
                            ...
                            <button
                              ...
                              className="p-1.5 text-content-secondary hover:bg-surface rounded"
                            >
                          ...
                          <div className="flex items-center gap-2 group">
                            <div className="flex-1 text-sm text-content-secondary">
                              {member.characters.length > 0 ? (
                                member.characters
                                  .map(
                                    (c) =>
                                      `${c.character_name}${c.level ? ` (${c.level})` : ""}`,
                                  )
                                  .join(", ")
                              ) : (
                            ...
                            <button
                              onClick={() => {
                                ...
                                setNewCharacterName(
                                  member.characters[0]?.character_name || "",
                                );
                              }}
                      ...
                      <td className="p-3">
                        <span
                          className={`text-xs px-2 py-1 rounded ${member.is_active ? "bg-success/15 text-success" : "bg-surface text-content-muted"}`}
                        >
                      ...
                      <td className="p-3">
                        <div className="flex items-center justify-end gap-1">
                          {editingUser === member.id ? (
                            <>
                              <button
                                onClick={() => void handleUpdateUser(member.id)}
                                className="p-1.5 text-success hover:bg-success/20 rounded"
                              >
                                <Save className="w-3.5 h-3.5" />
                              </button>
                            ...
                            <>
                              <button
                                ...
                                className="p-1.5 text-info hover:bg-info/20 rounded"
                                title="Edit"
                              >
                              ...
                              <button
                                ...
                                className="p-1.5 text-danger hover:bg-danger/20 rounded disabled:opacity-30"
                                title="Delete"
                              >
        ...
        <div className="space-y-2">
          {events.length === 0 ? (
            <div className="text-center py-12 text-content-muted bg-surface-base/50 border border-line rounded-lg">
              No events found.
            </div>
          ) : (
            events.map((event) => (
              <div
                key={event.id}
                className="bg-surface-base/50 border border-line rounded-lg p-4 flex items-start justify-between gap-4"
              >
                <div>
                  <div className="font-medium text-content-primary text-sm">
                    {event.title}
