...
export default function LocationDetailPage() {
  ...
  const backTarget = resolveCyclopediaReturnTarget(
    ...
    '/cyclopedia?tab=zones',
  );
  const cyclopediaState = createCyclopediaRouteState(backTarget);
  return <Page variant="focused"><div>
    <button onClick={() => navigate(backTarget)} className="mb-6 flex min-h-11 items-center gap-2 text-content-secondary hover:text-content-primary"><ArrowLeft size={18} />{t('namedKnowledge.back')}</button>
    <article className="rounded-2xl border border-line bg-surface-base/70 p-4 sm:p-6">
      ...
      {place.relationships.length > 0 && <section className="mt-6 rounded-xl border border-line p-4"><h2 className="mb-3 font-semibold text-primary">{t('namedKnowledge.relationships')}</h2><ul className="space-y-2">{place.relationships.map((relationship, index) => { const path = relationshipPath(relationship); const content = <><span><span className="text-xs text-content-muted">{t(`knowledgeGraph.relationships.${relationship.relationship_type}`)}</span><span className="block text-content-primary">{relationship.target_name}</span></span>{path && <ArrowUpRight size={15} className="text-primary" />}</>; return <li key={`${relationship.relationship_type}-${relationship.target_name}-${index}`}>{path ? <Link to={path} state={cyclopediaState} className="flex min-h-11 items-center justify-between rounded-lg bg-surface-base/60 px-3 py-2 hover:ring-1 hover:ring-primary/40">{content}</Link> : <div className="flex min-h-11 items-center rounded-lg bg-surface-base/60 px-3 py-2">{content}</div>}</li>; })}</ul></section>}
      <MapMetadataPanel locationIdentifier={place.slug} mapTarget={{ entityType: 'location', name: place.name, slug: place.slug, canonicalEntityId: place.knowledge_entity_id }} />
