...
import { BookOpenCheck, ChevronLeft, ChevronRight, Crown, Info, MapPinned, Search, Sparkles, Swords, X } from 'lucide-react';
...
export default function TibiaMapPage() {
  ...
  const visiblePage = visible.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE);
  ...
  return <div className="relative left-1/2 w-[calc(100vw-1rem)] -translate-x-1/2 py-3 sm:w-[calc(100vw-2rem)]">
    ...
    <div className="grid gap-3 lg:h-[calc(100vh-12rem)] lg:min-h-[36rem] lg:grid-cols-[20rem_minmax(0,1fr)]">
      <section className="relative order-1 min-h-[32rem] overflow-hidden rounded-2xl border border-line bg-surface-base lg:col-start-2 lg:row-start-1">
      ...
      <aside className="order-2 flex min-h-0 flex-col rounded-2xl border border-line bg-surface-raised p-3 lg:col-start-1 lg:row-start-1">
        ...
        {!query.trim() ? <div className="mb-3 rounded-xl border border-line bg-surface p-3"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-content-muted">Quick searches</p><div className="flex flex-wrap gap-1.5">{['Thais', 'Edron', 'Dragon', 'Demon', 'Quest'].map((shortcut) => <button key={shortcut} type="button" onClick={() => setQuery(shortcut)} className="rounded-full border border-line px-2.5 py-1 text-xs text-content-secondary hover:border-primary/40 hover:text-primary">{shortcut}</button>)}</div></div> : null}
        <div className="min-h-0 flex-1 space-y-2 overflow-y-auto" aria-live="polite">
          ...
          {!loading && query.trim().length >= 2 && !visible.length ? <p className="rounded-xl border border-line bg-surface p-4 text-sm text-content-muted">{t('map.noResults')}</p> : null}
          {visiblePage.map((row) => { const Icon = layerIcons[row.entity_type]; return <button key={row.id} type="button" onClick={() => setSelected(row)} className={`flex w-full items-start gap-3 rounded-xl border p-2.5 text-left transition ${selected?.id === row.id ? 'border-primary/50 bg-primary/10' : 'border-line bg-surface hover:bg-surface-active'}`}><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary"><Icon className="size-4" /></span><span className="min-w-0"><strong className="block truncate text-sm text-content-primary">{row.name}</strong><small className="block truncate text-xs text-content-muted">{row.subtitle || t(`map.layers.${row.entity_type}`)}</small><small className={`mt-0.5 block text-[10px] ${row.geometry_status === 'mapped' ? 'text-success' : 'text-content-muted'}`}>{row.geometry_status === 'mapped' ? t('map.mapped') : t('map.knowledgeOnly')}</small></span></button>; })}
        </div>
        {visible.length > PAGE_SIZE ? <div className="mt-2 flex items-center justify-between border-t border-line pt-2 text-xs text-content-muted"><button type="button" disabled={page === 0} onClick={() => setPage((value) => Math.max(0, value - 1))} className="grid size-9 place-items-center rounded-lg border border-line disabled:opacity-35" aria-label="Previous results"><ChevronLeft className="size-4" /></button><span>{page + 1} / {pageCount}</span><button type="button" disabled={page + 1 >= pageCount} onClick={() => setPage((value) => Math.min(pageCount - 1, value + 1))} className="grid size-9 place-items-center rounded-lg border border-line disabled:opacity-35" aria-label="Next results"><ChevronRight className="size-4" /></button></div> : null}
      </aside>
    </div>
  </div>;
}
