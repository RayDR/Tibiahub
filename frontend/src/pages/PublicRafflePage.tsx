...
export default function PublicRafflePage() {
  ...
  const [stage, setStage] = useState<
    "waiting" | "drawing" | "elimination1" | "elimination2" | "winner"
  >("waiting");
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const event = publicCode
          ? await eventsApi.getPublicEventByCode(publicCode)
          : await eventsApi.getPublicEvent(uuid!);
        setEventData(event);

        // Get participants status
        const status = await eventsApi.getRaffleStatus(
          publicCode ? event.uuid : uuid!,
        );
  ...
  if (loading)
    return (
      <div className="h-screen flex items-center justify-center bg-surface-base text-content-primary">
        <Loader2 className="animate-spin w-10 h-10 text-primary" />
      </div>
    );
  if (!eventData)
    return (
      <div className="text-content-primary text-center mt-20">
        {t("raffle.eventNotFound", "Event not found")}
      </div>
    );
    ...
    <div className="min-h-screen bg-surface-base text-content-primary font-sans selection:bg-primary/30">
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-8">
        <header className="text-center mb-8">
          <h1 className="mb-2 bg-gradient-to-r from-primary to-accent bg-clip-text font-serif text-4xl font-bold tracking-wider text-transparent md:text-6xl">
            {eventData.title}
          </h1>
          <p className="text-primary/80 text-lg uppercase tracking-widest">
            {eventData.reward}
          </p>
        </header>
        ...
        {stage === "winner" && winner ? (
          ...
          >
            ...
            <h2 className="text-2xl text-primary mb-2">
              {t("raffle.winner", "WINNER")}
            </h2>
            <div className="text-5xl md:text-7xl font-bold text-content-primary mb-6">
              {winner.name}
            </div>
            <p className="text-content-secondary text-lg">
              {t(
                "raffle.congratsMessage",
                "Congratulations! Your prize awaits.",
              )}
            </p>
            <p className="text-primary text-sm mt-4 italic">
              {t(
                "raffle.winnerMessage",
                '"Fortune favors the brave. Your name has been chosen by destiny!"',
              )}
            </p>
          </motion.div>
        ) : (
            ...
            <div className="space-y-6">
              <div className="bg-surface-base/40 border border-line p-6 rounded-2xl backdrop-blur-sm">
                ...
                <div className="text-3xl font-mono text-content-primary mb-2">
                  {stage === "waiting"
                    ? timeLeft || t("raffle.liveEvent", "Live Event")
                    : t("raffle.drawing", "DRAWING...")}
                </div>
                <p className="text-sm text-content-secondary">
                  {stage === "waiting"
                    ? t(
                        "raffle.autoUpdate",
                        "The event changes will appear automatically. Sit tight!",
                      )
                    : t(
                        "raffle.drawingProcess",
                        "The raffle mechanism is running...",
                      )}
                </p>
                ...
                {user?.is_superuser && stage === "waiting" && !winner && (
                  <div className="mt-8 pt-4 border-t border-line">
                    <button
                      onClick={() =>
                        (eventData?.uuid || uuid) &&
                        eventsApi.autoDrawRaffle(
                          (eventData?.uuid || uuid) as string,
                        )
                      }
                  ...
                  {participants.map((p, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          className={`truncate ${isEliminated ? "text-danger" : "text-content-secondary"}`}
                        >
                          {p.name}
                        </span>
                      </div>
                  ...
                  <div className="relative z-10 text-center">
                    <h3 className="text-2xl text-primary mb-8 uppercase tracking-widest font-bold">
                      {stage.includes("elimination")
                        ? t("raffle.eliminating", "ELIMINATING...")
                        : t("raffle.drawingWinner", "DRAWING WINNER")}
                    </h3>
      ...
      <footer className="fixed bottom-0 w-full text-center p-4 text-xs text-content-muted bg-surface-base/80 backdrop-blur">
        {t("raffle.transparencyId", "Transparency ID")}: {eventData.id} |{" "}
        {t("raffle.timestamp", "Timestamp")}: {new Date().toISOString()}
      </footer>
    </div>
  );
}
