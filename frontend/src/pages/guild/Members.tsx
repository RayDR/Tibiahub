  ...
  return <div className="space-y-5">
    ...
    {error ? <EmptyState title={t('guildMembers.error')} description={t('guildMembers.errorHelp')} action={<AppButton onClick={() => void load()}>{t('common.retry')}</AppButton>} /> : <>
      ...
      {visible.length === 0 ? <EmptyState title={query ? t('guildMembers.noMatches') : t('guildMembers.empty')} description={query ? t('guildMembers.noMatchesHelp') : t('guildMembers.emptyHelp')} /> : <>
        ...
        <TableContainer className="responsive-data-table"><Table><thead><tr><th>{t('guildMembers.fields.character')}</th><th>{t('guildMembers.fields.level')}</th><th>{t('guildMembers.fields.vocation')}</th><th>{t('guildMembers.fields.rank')}</th><th>{t('identity.accountStatus')}</th><th>{t('guildMembers.fields.lastLogin')}</th></tr></thead><tbody>{visible.map(member => <tr key={`${member.character_name}-${member.snapshot_at}`}><td className="font-medium text-content-primary">{member.linked_username ? <Link className="text-primary hover:underline" to={`/members/${encodeURIComponent(member.linked_username)}`}>{member.character_name}</Link> : member.character_name}</td><td>{member.level ?? t('common.notAvailable')}</td><td>{member.vocation || t('common.unknown')}</td><td>{member.rank || member.role || t('guildMembers.values.member')}</td><td><Badge tone={member.linked_username ? 'success' : 'neutral'}>{t(member.linked_username ? 'identity.registered' : 'identity.unregistered')}</Badge></td><td className="text-sm text-content-muted">{member.last_login || t('common.unknown')}</td></tr>)}</tbody></Table></TableContainer>
      </>}
    </>}
  </div>;
}
...
function MemberCard({ member }: { member: GuildMember }) {
  const { t } = useTranslation();
  return <Card className="p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="font-semibold">{member.linked_username ? <Link className="text-primary hover:underline" to={`/members/${encodeURIComponent(member.linked_username)}`}>{member.character_name}</Link> : member.character_name}</h2><p className="text-sm text-content-muted">{member.vocation || t('common.unknown')}</p></div><Badge tone="primary">{t('guildMembers.levelValue', { level: member.level ?? t('common.notAvailable') })}</Badge></div><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs text-content-muted">{t('guildMembers.fields.rank')}</dt><dd>{member.rank || member.role || t('guildMembers.values.member')}</dd></div><div><dt className="text-xs text-content-muted">{t('identity.accountStatus')}</dt><dd>{t(member.linked_username ? 'identity.registered' : 'identity.unregistered')}</dd></div></dl></Card>;
}
