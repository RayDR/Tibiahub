...
export default function Announcements() {
    ...
    return (
        <div className="space-y-8">
            <div className="border-b-4 border-gradient-to-r from-primary to-primary pb-6">
                    ...
                    <div className="flex gap-3">
                        ...
                        {canCreate && (
                            <button
                                ...
                                className="flex items-center gap-2 bg-gradient-to-r from-primary to-primary hover:from-primary hover:to-primary text-content-primary px-4 py-3 rounded-lg transition-all font-semibold text-base shadow-lg hover:shadow-primary/30"
                            >
            ...
            {/* Filters Panel */}
            {showFilters && (
                <div className="bg-gradient-to-b from-surface/40 to-surface-base/40 border-2 border-line/60 rounded-xl p-6 backdrop-blur-sm">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                        <div>
                            <label className="block text-xs font-bold text-content-secondary mb-2 uppercase tracking-wider">{t('guild.type')}</label>
                            <select
                                value={filters.type}
                        ...
                        <div>
                            <label className="block text-xs font-bold text-content-secondary mb-2 uppercase tracking-wider">{t('guild.author')}</label>
                            <input
                                type="text"
                                value={filters.author}
                                onChange={e => setFilters({ ...filters, author: e.target.value })}
                                placeholder={t('common.search') + '...'}
                                className="w-full bg-surface-base/60 border-2 border-line/60 hover:border-line rounded-lg p-3 text-content-primary text-sm placeholder-content-muted0 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                            />
                        </div>

                        <div>
                            <label className="block text-xs font-bold text-content-secondary mb-2 uppercase tracking-wider">{t('guild.filterByDate')} (desde)</label>
                            <input
                                type="date"
                        ...
                        <div>
                            <label className="block text-xs font-bold text-content-secondary mb-2 uppercase tracking-wider">{t('guild.filterByDate')} (hasta)</label>
                            <input
                                type="date"
                ...
                <div className="space-y-6">
                    {filteredAnnouncements.map((ann, index) => {
                        ...
                        return (
                            <div
                                ...
                                className="bg-gradient-to-br from-surface/60 to-surface-base/80 border-2 border-line/80 rounded-xl overflow-hidden shadow-2xl hover:shadow-primary/20 hover:border-primary/60 transition-all duration-300 cursor-pointer animate-fade-in group hover:scale-[1.01]"
                            >
                                <div className={`h-2 w-full transition-all duration-300 ${ann.type === 'contest' ? 'bg-gradient-to-r from-accent to-accent' :
                                    ann.type === 'hunt' ? 'bg-gradient-to-r from-danger to-danger' :
                                ...
                                <div className="p-8">
                                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 mb-6">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <h2 className="text-2xl lg:text-3xl font-bold text-content-primary leading-tight group-hover:text-primary transition-colors break-words">{ann.title}</h2>
                                        </div>
                                    ...
                                    <div className="mt-4 pt-4 border-t border-line/50 flex items-center justify-end">
                                        <span className="text-xs text-content-muted group-hover:text-primary/70 transition-colors">Click para ver más →</span>
                                    </div>
                                </div>
                            </div>
                        );
                    ...
                    {!hasMore && announcements.length > 0 && (
                        <div className="text-center py-8">
                            <p className="text-sm text-content-muted font-medium">{t('guild.noMoreResults')}</p>
                        </div>
                    )}
                    ...
                    {!loading && announcements.length === 0 && (
                        <div className="text-center py-16">
                            ...
                            <p className="text-content-secondary text-lg font-medium">{t('guild.noAnnouncements')}</p>
                            <p className="text-content-muted text-sm mt-2">Vuelve más tarde para nuevos anuncios</p>
                        </div>
                    )}
                </div>
            )}
            ...
            {detailModal && (
                ...
                >
                    ...
                    >
                        <div className={`h-3 w-full ${detailModal.type === 'contest' ? 'bg-gradient-to-r from-accent to-accent' :
                            detailModal.type === 'hunt' ? 'bg-gradient-to-r from-danger to-danger' :
                                'bg-gradient-to-r from-primary to-primary'
                            }`} />
                        ...
                        <div className="sticky top-0 bg-surface-base/95 backdrop-blur p-8 border-b-2 border-line/50 flex items-start justify-between">
                            <div className="flex-1 pr-4">
                                ...
                                <h3 className="text-3xl lg:text-4xl font-bold text-content-primary leading-tight break-words">{detailModal.title}</h3>
                    ...
                    <div className="bg-gradient-to-b from-surface to-surface-base border-2 border-line/80 rounded-2xl w-full max-w-2xl shadow-2xl">
                        <div className="p-8 border-b-2 border-line/50">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <h3 className="text-2xl font-bold text-content-primary">{t('guild.create')} {t('guild.announcements')}</h3>
                            </div>
                            <p className="text-content-secondary text-sm">{t('guild.announcements')} ({t('guild.create')})</p>
                        </div>

                        <form onSubmit={handleCreate} className="p-8 space-y-6">
                            <div>
                                <label className="block text-sm font-bold text-content-secondary mb-3 uppercase tracking-wider">{t('guild.title')}</label>
                                <input
                                    type="text"
                                    required
                                    ...
                                    className="w-full bg-surface-base/80 border-2 border-line/60 rounded-lg px-4 py-3 text-content-primary placeholder-content-muted0 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all text-base"
                                />
                            </div>

                            <div>
                                <label className="block text-sm font-bold text-content-secondary mb-3 uppercase tracking-wider">{t('guild.type')}</label>
                                <select
                                    value={formData.type}
                            ...
                            <div>
                                <label className="block text-sm font-bold text-content-secondary mb-3 uppercase tracking-wider">{t('guild.content')}</label>
                                <textarea
                                    required
                                    rows={8}
                                    ...
                                    className="w-full bg-surface-base/80 border-2 border-line/60 rounded-lg px-4 py-3 text-content-primary placeholder-content-muted0 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-mono text-sm leading-relaxed"
                                />
                            </div>
                            ...
                            <div className="flex justify-end gap-4 mt-8 pt-6 border-t border-line/50">
                                ...
                                <button
                                    type="submit"
                                    disabled={creating}
                                    className="bg-gradient-to-r from-primary to-primary hover:from-primary hover:to-primary text-content-primary px-6 py-2.5 rounded-lg font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-all text-base shadow-lg hover:shadow-primary/30"
                                >
