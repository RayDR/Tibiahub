...
const checks = [
  ...
  [reference.includes('resolveLocalItem') && reference.includes('resolveLocalQuest') && reference.includes('resolution_status === \'resolved\''), 'Quest item and prerequisite references must resolve local exact knowledge only'],
  ...
  [detail.includes('window.location.hash') && !detail.includes('location.hash, location.search'), 'Quest mission hash navigation must not refetch the Quest payload'],
