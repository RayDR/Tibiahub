...
- `package.json` and `package-lock.json` are fingerprinted together.
...
The deploy should fail safely on unrecoverable dependency installation errors, but missing local dependencies are a recoverable preflight condition and should not require manual intervention.
